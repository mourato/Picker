...
"Pick a Color" (panel button or global hotkey, default **⌃⌥C** via Carbon `RegisterEventHotKey`) gates on **Screen Recording** (`CGPreflightScreenCaptureAccess` / `CGRequestScreenCaptureAccess`) before hiding the panel — toast + System Settings deep link on deny. On grant: freeze displays per `AppSettings.freezeScope` (default all connected displays, or only the display under the cursor), excluding Picker’s own windows; show one loupe overlay window per frozen `NSScreen`, then tuck the panel away under it. A successful click commits the color to the palette, copies it to the clipboard in `AppSettings.clipboardFormat` (HEX / RGB / HSL / HSB — independent of the loupe/hero `colorDisplayFormat`), and leaves the panel closed. Esc / right-click cancels and, if the panel was open when sampling started, reveals it *before* removing the overlay so the live desktop never flashes. The app activates briefly so Esc / − / = / ⌘− / ⌘= are delivered and consumed. Loupe zoom defaults to 12× (range 4…32; pixel grid from 8× when `AppSettings.showPixelGrid` is on, default on); loupe radius defaults to 72pt (48…160).
