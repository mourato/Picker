...
Picker is a native macOS menu-bar color picker (SwiftUI + AppKit, no dependencies). It samples any screen pixel, shows HEX/RGB/HSL, and keeps a saved palette.

## Build & run

- **Build with `./build.sh`, not `swift build`.** Plain SPM only compiles the binary. `build.sh` also assembles `build/Picker.app`, generates `Contents/Info.plist` (`LSUIElement = true` for menu-bar-only, `LSMinimumSystemVersion = 26.0`), and **ad-hoc codesigns** (`codesign --force --deep --sign -`). The Info.plist and signing are required for Liquid Glass and the `NSColorSampler` loupe to work.
...
macOS 26 (Tahoe). The app relies on SwiftUI `glassEffect` (Liquid Glass) and `NSColorSampler`. There is no fallback for older macOS.

## Don't "fix" these on sight — they're deliberate

- **Non-activating `NSPanel`, not `MenuBarExtra`.** `FloatingPanel` sets `canBecomeMain = false` and a non-activating style so the panel stays open *during* sampling and doesn't steal focus from the app being sampled. `MenuBarExtra` would close on sample.
...
Both stores persist JSON to `UserDefaults`, capped at 60 entries: colors under `picker.pickedColors.v1` (consecutive duplicate samples update rather than append), fonts under `picker.pickedFonts.v1` (re-grabbing a family moves it to the front). The `--demo` path sets `persistenceEnabled = false` on both so seeded data never touches the user's real saved items.
...
- The Colors/Fonts `SectionSwitch` slides a single pill (`GeometryReader` + offset); the labels are pinned in fixed slots and only crossfade color — they never reflow.
